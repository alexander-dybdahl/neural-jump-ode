"""
Hybrid OU-BS experiment: OU from [0, switch_time] and Black-Scholes from [switch_time, T].
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Hybrid OU-BS Neural Jump ODE Experiment')
    ...
    # OU parameters (first phase)
    parser.add_argument('--theta-ou', type=float, default=1.0, help='OU mean reversion speed')
    parser.add_argument('--mu-ou', type=float, default=0.0, help='OU long-term mean')
    parser.add_argument('--sigma-ou', type=float, default=0.3, help='OU volatility')
    
    # Black-Scholes parameters (second phase)
    parser.add_argument('--mu-bs', type=float, default=0.1, help='BS drift parameter')
    parser.add_argument('--sigma-bs', type=float, default=0.5, help='BS volatility parameter')
    
    # Hybrid parameters
    parser.add_argument('--switch-time', type=float, default=None, 
                        help='Time to switch from OU to BS (if None, random in [0.2*T, 0.8*T])')
...
def main():
    """Run a Hybrid OU-BS experiment."""
    args = parse_args()
    
    # Experiment configuration with command-line overrides
    config = {
        "experiment_name": "njode_hybrid_ou_bs",
        "input_dim": 1,
        ...
        "data": {
            "process_type": "hybrid_ou_bs",
            "n_train": args.n_train,
            "n_val": args.n_val,
            "obs_fraction": args.obs_fraction,
            "cache_data": args.cache_data,
            "theta_ou": args.theta_ou,
            "mu_ou": args.mu_ou,
            "sigma_ou": args.sigma_ou,
            "mu_bs": args.mu_bs,
            "sigma_bs": args.sigma_bs,
    ...
    plot_single_trajectory_with_condexp(
        ...
        process_type="hybrid_ou_bs",
        process_params={
            "theta_ou": config["data"]["theta_ou"],
            "mu_ou": config["data"]["mu_ou"],
            "sigma_ou": config["data"]["sigma_ou"],
            "mu_bs": config["data"]["mu_bs"],
            "sigma_bs": config["data"]["sigma_bs"],
            "switch_time": config["data"]["switch_time"],
            "T": config["data"]["T"],
